		...
		<section id="latest-recipes" class="mt-10 space-y-6 lg:mt-12">
			<div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
				<div class="space-y-3">
					...
					<h2 class="font-display text-3xl text-stone-900 dark:text-night-text md:text-4xl">Verse ideeën voor vanavond</h2>
					<p class="max-w-2xl text-sm leading-7 text-stone-600 dark:text-night-muted md:text-base">De nieuwste recepten staan nu sneller in beeld, zodat bezoekers direct kunnen kiezen.</p>
				</div>
				<?php if ( $recipe_archive_url ) : ?>
					<a href="<?php echo esc_url( $recipe_archive_url ); ?>" class="inline-flex items-center gap-2 self-start rounded-full border border-rose-200 bg-white/80 px-5 py-3 text-sm font-semibold text-stone-700 transition hover:bg-rose-50 dark:border-night-border dark:bg-night-surface/80 dark:text-night-textSoft dark:hover:bg-night-surfaceElevated">Naar het receptenoverzicht <span aria-hidden="true">&rarr;</span></a>
				<?php endif; ?>
			</div>
			...
			<?php if ( ! empty( $latest_recipes ) ) : ?>
				<div class="grid gap-6 lg:grid-cols-3">
					...
					foreach ( $latest_recipes as $recept ) :
						...
						$title_class     = 0 === $card_index ? 'font-display text-3xl leading-tight text-stone-900 dark:text-night-text md:text-4xl' : 'font-display text-2xl leading-tight text-stone-900 dark:text-night-text';
						$description_text = wp_trim_words( (string) ( $recept['beschrijving'] ?? '' ), 24 );
						?>
						<article id="recept-<?php echo esc_attr( $recept['id'] ); ?>" class="group overflow-hidden rounded-[2rem] border border-white/80 bg-white/90 shadow-card backdrop-blur transition duration-300 hover:-translate-y-1 hover:border-rose-200 dark:border-night-border dark:bg-night-surface/90 dark:hover:border-night-borderStrong <?php echo esc_attr( $card_class ); ?>">
							<a href="<?php echo esc_url( bellas_kitchen_get_recept_url( $recept ) ); ?>" class="<?php echo esc_attr( $link_class ); ?> h-full">
								<div class="<?php echo esc_attr( $image_class ); ?> overflow-hidden bg-rose-50">
									...
									<?php else : ?>
										<div class="flex h-full items-center justify-center bg-slate-100 text-5xl text-stone-900 dark:bg-night-surfaceElevated dark:text-night-text">&#127869;</div>
									<?php endif; ?>
								</div>
								<div class="<?php echo esc_attr( $card_body_class ); ?>">
									<div class="space-y-4">
										...
										<p class="text-sm leading-7 text-stone-600 dark:text-night-muted md:text-base">
											<?php echo esc_html( $description_text ); ?>
										</p>
									</div>

									<div class="space-y-4">
										<div class="flex flex-wrap items-center gap-3 text-sm text-stone-700 dark:text-night-muted">
											<?php if ( ! empty( $recept['bereidingstijd'] ) ) : ?>
												<span class="inline-flex items-center rounded-full border border-rose-100 bg-rose-50 px-3 py-2 dark:border-night-border dark:bg-night-surfaceElevated"><?php echo esc_html( bellas_kitchen_format_recept_duration( (int) $recept['bereidingstijd'] ) ); ?></span>
											<?php endif; ?>
											<span class="inline-flex items-center rounded-full border border-transparent bg-slate-100 px-3 py-2 text-stone-800 dark:bg-night-surfaceElevated dark:text-night-text">Bekijk recept</span>
										</div>
			...
			<?php else : ?>
				<div class="rounded-[2rem] border border-dashed border-rose-200 bg-white/80 p-8 text-center text-stone-600 shadow-card dark:border-night-border dark:bg-night-surface/90 dark:text-night-muted">
					<p class="font-display text-3xl text-stone-900 dark:text-night-text">Nog geen recepten gepubliceerd</p>
